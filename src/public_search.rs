    ...
    for task in tasks.into_iter().take(max_tasks) {
        let findings = match task.kind {
            PublicSearchTaskKind::GitHubUserSearch if github_enabled => {
                ...
                search_github_users(&client, &task).await.unwrap_or_else(|_| {
            ...
            PublicSearchTaskKind::GitHubRepoSearch if github_enabled => {
                report.stats.tasks_executed += 1;
                search_github_repos(&client, &task).await.unwrap_or_else(|_| {
                    report.stats.api_errors += 1;
                    Vec::new()
                })
            }
        ...
        for finding in findings {
            if let Some(finding) = apply_noise_rules_to_finding(finding, &mut report.stats) {
...
fn apply_noise_rules_to_finding(mut finding: PublicSearchFinding, stats: &mut PublicSearchStats) -> Option<PublicSearchFinding> {
...
pub fn save_public_search_report(report: &PublicSearchReport, path: &str) -> Result<(), String> {
    let json = serde_json::to_string_pretty(report).map_err(|err| format!("serialize public search report: {}", err))?;
...
async fn search_github_users(client: &Client, task: &PublicSearchTask) -> Result<Vec<PublicSearchFinding>, reqwest::Error> {
    let url = format!("https://api.github.com/search/users?q={}&per_page=5", url_encode(&task.query));
    let body = fetch_json(client, &url).await?;
    let mut findings = Vec::new();
    if let Some(items) = body.get("items").and_then(|v| v.as_array()) {
        for item in items.iter().take(5) {
            if let Some(login) = item.get("login").and_then(|v| v.as_str()) {
                let html_url = item.get("html_url").and_then(|v| v.as_str()).map(|s| s.to_string());
                findings.push(PublicSearchFinding { task_id: task.task_id.clone(), source_id: task.source_id.clone(), entity_type: EntityType::Username, value: login.to_string(), confidence: confidence_for_github_match(login, &task.query), url: html_url.clone(), note: "github_user_search_login".to_string() });
                if let Some(url) = html_url {
                    findings.push(PublicSearchFinding { task_id: task.task_id.clone(), source_id: task.source_id.clone(), entity_type: EntityType::Url, value: url.clone(), confidence: 55, url: Some(url), note: "github_user_profile_url".to_string() });
                }
            }
        }
    }
...
async fn search_github_repos(client: &Client, task: &PublicSearchTask) -> Result<Vec<PublicSearchFinding>, reqwest::Error> {
    let url = format!("https://api.github.com/search/repositories?q={}&per_page=5", url_encode(&task.query));
    let body = fetch_json(client, &url).await?;
    let mut findings = Vec::new();
    if let Some(items) = body.get("items").and_then(|v| v.as_array()) {
        for item in items.iter().take(5) {
            if let Some(full_name) = item.get("full_name").and_then(|v| v.as_str()) {
                ...
                findings.push(PublicSearchFinding { task_id: task.task_id.clone(), source_id: task.source_id.clone(), entity_type: EntityType::Url, value: repo_url.clone(), confidence: confidence_for_github_match(full_name, &task.query).saturating_sub(10), url: Some(repo_url), note: "github_repo_search_result".to_string() });
                if let Some(owner) = item.get("owner").and_then(|v| v.get("login")).and_then(|v| v.as_str()) {
                    let html_url = item.get("html_url").and_then(|v| v.as_str()).map(|s| s.to_string());
                    findings.push(PublicSearchFinding { task_id: task.task_id.clone(), source_id: task.source_id.clone(), entity_type: EntityType::Username, value: owner.to_string(), confidence: 45, url: html_url, note: "github_repo_owner".to_string() });
                }
            }
        }
    }
...
async fn fetch_json(client: &Client, url: &str) -> Result<Value, reqwest::Error> {
    client.get(url).header("User-Agent", "XGEN-PublicSearch/1.0 (+local research)").header("Accept", "application/vnd.github+json").send().await?.json::<Value>().await
}
...
fn search_terms_from_seed(seed: &EntityNode) -> Vec<String> {
    ...
    match &seed.entity_type {
        EntityType::Nickname | EntityType::Username => {
            if let Some(username) = normalize_username(&seed.value) { terms.push(username); }
        }
        EntityType::Email => {
            let email = seed.value.trim().to_lowercase();
            if is_email(&email) {
                terms.push(email.clone());
                if let Some((local, _)) = email.split_once('@') { terms.extend(username_candidates_from_email_local(local)); }
            }
        }
        EntityType::FullName => {
            let name = seed.value.trim();
            if !name.is_empty() { terms.push(name.to_string()); terms.push(transliterate_basic(name)); }
        }
        EntityType::Url | EntityType::Domain => terms.push(seed.value.trim().to_string()),
        _ => {}
    }
...
fn username_candidates_from_email_local(local: &str) -> Vec<String> {
    ...
    if let Some(username) = normalize_username(&base) { candidates.push(username); }
    let compact = base.replace(['.', '-', '_'], "");
    if compact != base {
        if let Some(username) = normalize_username(&compact) { candidates.push(username); }
    }
    candidates.sort();
    candidates.dedup();
    candidates
...
fn normalize_username(raw: &str) -> Option<String> {
    ...
    if username.is_empty() || username.len() > 64 || lowered.starts_with("seed_") || username.contains(':') || username.contains('/') || username.contains('\\') || username.contains(' ') {
        return None;
    }
    if username.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '.' || c == '-') { Some(username.to_string()) } else { None }
}

fn is_email(token: &str) -> bool {
    let token = token.trim();
    let parts: Vec<&str> = token.split('@').collect();
    if parts.len() != 2 || parts[0].is_empty() || parts[1].is_empty() { return false; }
    let domain = parts[1];
    domain.contains('.') && !domain.starts_with('.') && !domain.ends_with('.') && token.chars().all(|c| c.is_ascii_alphanumeric() || matches!(c, '@' | '.' | '_' | '-' | '+'))
}

fn confidence_for_github_match(value: &str, query: &str) -> u8 {
    let value = value.to_lowercase();
    let query = query.to_lowercase();
    if value == query { 75 } else if value.contains(&query) || query.contains(&value) { 60 } else { 40 }
}
...
fn sensitivity_for(entity_type: &EntityType) -> SensitivityClass {
    match entity_type { EntityType::Email | EntityType::Phone => SensitivityClass::Personal, _ => SensitivityClass::PublicLow }
}
...
fn transliterate_basic(value: &str) -> String {
    value.chars().map(|ch| match ch {
        'а' | 'А' => "a", 'б' | 'Б' => "b", 'в' | 'В' => "v", 'г' | 'Г' => "g", 'д' | 'Д' => "d", 'е' | 'Е' | 'ё' | 'Ё' => "e", 'ж' | 'Ж' => "zh", 'з' | 'З' => "z", 'и' | 'И' => "i", 'й' | 'Й' => "y", 'к' | 'К' => "k", 'л' | 'Л' => "l", 'м' | 'М' => "m", 'н' | 'Н' => "n", 'о' | 'О' => "o", 'п' | 'П' => "p", 'р' | 'Р' => "r", 'с' | 'С' => "s", 'т' | 'Т' => "t", 'у' | 'У' => "u", 'ф' | 'Ф' => "f", 'х' | 'Х' => "h", 'ц' | 'Ц' => "ts", 'ч' | 'Ч' => "ch", 'ш' | 'Ш' => "sh", 'щ' | 'Щ' => "sch", 'ы' | 'Ы' => "y", 'э' | 'Э' => "e", 'ю' | 'Ю' => "yu", 'я' | 'Я' => "ya", 'ь' | 'Ь' | 'ъ' | 'Ъ' => "", _ => "" }).collect::<Vec<_>>().join("").split_whitespace().collect::<Vec<_>>().join(" ")
}
...
mod tests {
    ...
    #[test]
    fn repo_owner_is_downranked_by_noise_rules() {
        let finding = PublicSearchFinding { task_id: "t".to_string(), source_id: "github_public_repo_search".to_string(), entity_type: EntityType::Username, value: "owner".to_string(), confidence: 45, url: Some("https://github.com/owner/repo".to_string()), note: "github_repo_owner".to_string() };
        let mut stats = PublicSearchStats::default();
        let filtered = apply_noise_rules_to_finding(finding, &mut stats).expect("downrank, not block");
        assert_eq!(stats.downranked_by_noise_rules, 1);
        assert!(filtered.confidence < 45);
    }
}
