...
pub fn sha256_hex(value: &str) -> String {
    hex_bytes(&sha256_bytes(value.as_bytes()))
}
...
mod tests {
    ...
    #[test]
    fn streaming_hash_matches_single_update() {
        let mut stream = Sha256Hasher::new();
        stream.update(b"ab");
        stream.update(b"c");
        assert_eq!(stream.finalize_hex(), sha256_hex("abc"));
    }
}
